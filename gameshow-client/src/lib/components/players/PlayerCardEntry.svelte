...
<div class="w-full flex {isRightSide ? 'flex-row-reverse' : 'flex-row'} justify-end items-center">
	<div
		class="w-4/6 {$buzzerTimeRemaining != null && isPressed
			? 'pulseBuzzer'
			: ''} aspect-video bg-purple-600 rounded-t-3xl rounded-b-lg border border-t-8 border-x-4 border-purple-600 flex flex-col"
	>
	...
	{:else}
		<div
			class="text-3xl font-bold py-3 px-10 bg-transparent text-transparent rounded-3xl mt-72 mx-16"
		>
			Buzzer
		</div>
	{/if}
